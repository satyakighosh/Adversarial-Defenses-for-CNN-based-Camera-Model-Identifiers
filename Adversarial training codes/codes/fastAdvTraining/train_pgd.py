...
from utils import (IMG_SIZE, attack_pgd, initialize_model_201, upper_limit, lower_limit, std, clamp, get_loaders,
    evaluate_pgd, evaluate_standard, set_parameter_requires_grad)
...
def main():
    ...
    model_ft, input_size = initialize_model_201(model_name, num_classes, feature_extract, use_pretrained=True)
    model = model_ft.cuda()
    model.train()
...
# normal acc only around 0.2 : so trying more complex model 201, and decreasing lr