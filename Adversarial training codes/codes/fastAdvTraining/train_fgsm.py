...
def get_args():
    ...
    parser.add_argument('--lr-max', default=0.02, type=float)
...
def main():
    ...
    # model = torch.hub.load('pytorch/vision:v0.10.0', 'densenet121', pretrained=True).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        # print(f'epoch, epoch_time - start_epoch_time, lr, train_loss/train_n, train_acc/train_n : {epoch}, {epoch_time - start_epoch_time}, {scheduler.get_last_lr()}, {train_loss/train_n}, {train_acc/train_n}')
        logger.info('%d \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f',
            epoch, epoch_time - start_epoch_time, lr, train_loss/train_n, train_acc/train_n, prev_robust_acc)
        torch.save({
        'epoch': epoch,
    ...
    print('Evaluation done. Check output.log file for accuracy values.')
...
# currently reduced lr to 0.02 from 0.2
# increased batchsize to 128 -> gpu error -> back to 32